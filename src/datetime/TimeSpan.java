...
public class TimeSpan {
    ...
    public TimeSpan(final int days, final int hours, final int minutes, final int seconds, final int milliseconds) {
        long num = ((long) days * 3600L * 24 + (long) hours * 3600L + (long) minutes * 60L + seconds)
                * TimeConstants.millisPerSecond + milliseconds;
    ...
    private long ticks;
    /*
     * private static volatile boolean legacyConfigChecked; private static volatile
     * boolean legacyMode;
     */
    ...
    private static TimeSpan interval(final double value, final int scale) {
        ...
        if (num2 > (double) TimeConstants.maxMilliSeconds || num2 < (double) TimeConstants.minMilliSeconds) {
            throw new Error();
        }
    ...
    /*
     * private double ticksToOADate(long value) { if (value == 0L) { return 0.0; }
     * if (value < TimeConstants.ticksPerDay) { value += TimeConstants.maxSeconds; }
     * if (value < TimeConstants.oaDateMinAsTicks) { throw new Error(); } long num =
     * (value - TimeConstants.maxSeconds) / 10000; if (num < 0) { long num2 = num %
     * TimeConstants.millisPerDay; if (num2 != 0L) { num -=
     * (TimeConstants.millisPerDay + num2) * 2; } } return (double)num /
     * (double)TimeConstants.millisPerDay; }
     */
